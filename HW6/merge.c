...
int main(int argc, char *argv[]) {
    if (argc != 5) {
        printf("Usage: ./Merge params_p.txt params_g.txt group1_secrets.txt group2_secrets.txt\n");
        return 1;
    }
    ...
    char **group1Lines = read_lines(argv[3], &count1);
    char **group2Lines = read_lines(argv[4], &count2);

    int totalCount = count1 + count2;

    BIGNUM **allSecrets = (BIGNUM **)malloc(totalCount * sizeof(BIGNUM *));
    int index = 0;

    for (int i = 0; i < count1; i++) {
        allSecrets[index] = NULL;
        BN_hex2bn(&allSecrets[index], group1Lines[i]);
        index++;
    }

    for (int i = 0; i < count2; i++) {
        allSecrets[index] = NULL;
        BN_hex2bn(&allSecrets[index], group2Lines[i]);
        index++;
    }

    leafBKs = (BIGNUM **)malloc(totalCount * sizeof(BIGNUM *));
    internalBKs = (BIGNUM **)malloc(totalCount * sizeof(BIGNUM *));

    leafIndex = 0;
    internalIndex = 0;

    Node *root = build_tree(allSecrets, totalCount, g, p, ctx);

    FILE *groupFile = fopen("group_key_merge.txt", "w");
    if (!groupFile) {
        printf("Error writing group_key_merge.txt\n");
        return 1;
    }
    ...
    FILE *blindFile = fopen("blinded_keys_merge.txt", "w");
    if (!blindFile) {
        printf("Error writing blinded_keys_merge.txt\n");
        return 1;
    }
