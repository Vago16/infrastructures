...
int main(int argc, char *argv[]) {
    if (argc != 6) {
        printf("Usage: ./Leave params_p.txt params_g.txt member_secrets.txt leaving_index.txt sponsor_new_secret.txt\n");
        return 1;
    }
    ...
    char *leaveIndexStr = read_file(argv[4]);
    int leaveIndexNum = atoi(leaveIndexStr);

    char *sponsorSecretStr = read_file(argv[5]);
    BIGNUM *sponsorNewSecret = NULL;
    BN_hex2bn(&sponsorNewSecret, sponsorSecretStr);

    int sponsorIndex;
    if (leaveIndexNum % 2 == 0) {
        sponsorIndex = leaveIndexNum + 1;
    } else {
        sponsorIndex = leaveIndexNum - 1;
    }

    BIGNUM **newSecrets = (BIGNUM **)malloc((count - 1) * sizeof(BIGNUM *));
    int j = 0;
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (i == sponsorIndex) {
            newSecrets[j] = BN_dup(sponsorNewSecret);
        } else {
            newSecrets[j] = BN_dup(secrets[i]);
        }
        j++;
    }
    ...
    leafBKs = (BIGNUM **)malloc(newCount * sizeof(BIGNUM *));
    internalBKs = (BIGNUM **)malloc(newCount * sizeof(BIGNUM *));

    leafIndex = 0;
    internalIndex = 0;

    Node *root = build_tree(newSecrets, newCount, g, p, ctx);

    FILE *groupFile = fopen("group_key_leave.txt", "w");
    if (!groupFile) {
        printf("Error writing group_key_leave.txt\n");
        return 1;
    }
    ...
    FILE *blindFile = fopen("blinded_keys_leave.txt", "w");
    if (!blindFile) {
        printf("Error writing blinded_keys_leave.txt\n");
        return 1;
    }
